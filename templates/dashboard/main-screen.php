        ...
        <div class="panel-dash-views">
            <div class="row">
				<?php
					...
					global $wpdb;
					$totalViewsCounts = 0;
					$resultViews = $wpdb->get_row('SELECT sum(count) as totalviews FROM '.$wpdb->prefix.'listing_stats_views WHERE user_id="'.$userID.'"');
					if(!empty($resultViews)){
						$totalViewsCounts = $resultViews->totalviews;
					...
					$resultReviews = $wpdb->get_row('SELECT sum(count) as totalviews FROM '.$wpdb->prefix.'listing_stats_reviews WHERE user_id="'.$userID.'"');
					if(!empty($resultReviews)){
						$totalReviewsCounts = $resultReviews->totalviews;
					...
					$totalLeadsCounts = 0;
					$resultLeads = $wpdb->get_row('SELECT sum(count) as totalviews FROM '.$wpdb->prefix.'listing_stats_leads WHERE user_id="'.$userID.'"');
					if(!empty($resultLeads)){
						$totalLeadsCounts = $resultLeads->totalviews;
                ...
                <div class="clearfix col-md-12 lp-stats-sorting-outer">
                    <div class="pull-left"><h4><?php echo date(get_option( 'date_format' )); ?></h4></div>
                    <div class="pull-right">
                        <ul class="lp_stats_duration_filter clearfix" style="display:none">
                            <li>
                                <button type="button" class="lp_stats_duratonBtn" data-chartduration="weekly" data-type=""><?php echo esc_html__('Weekly', 'listingpro'); ?></button>
                            </li>
                            <li>
                        ...
                        <div class="lp-new-activity-container clearfix">
                            ...
                            <div class="lp-new-activity-outer">
                                ...
                                foreach($recent_activities as $key=>$val){
                                    ...
                                    $lpOnlyDate = date('l, F j, Y', strtotime($datePosted));

                                    //$lpOnlyDate = date(get_option( 'date_format' ), strtotime($lpOnlyDate));
                                    $lpOnlyTime = date('H:i:s', strtotime($datePosted));

                                    $lpOnlyTime = date(get_option( 'time_format' ), strtotime($lpOnlyTime));
